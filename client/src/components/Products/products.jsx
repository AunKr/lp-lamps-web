...
useEffect(()=>{
  ...
  .then((res)=>{
    ...
    setBikeData(bike)
    const scooty = res?.rows?.filter(value => value.category === 'scooty')
    setScootyData(scooty)
    const rickshaw = res?.rows?.filter(value => value.category === 'rickshaw')
      ...
      <div className='container'>
        <div className='row'>
          ...
          <div className='col-lg-10'>
            <div className='row'>
              {bikeData && bikeData.length ? (
                bikeData.map((bike)=> {
                  return <div className='col-lg-4 col-md-6 portfolio-item'>
